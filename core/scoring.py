...
def calculate_final_assessment(metadata_results, watermark_results, ai_results, forensic_results):
    ...
    if metadata_results and (metadata_results.get("c2pa_verified") or metadata_results.get("evidence_score", 0) > 0.9):
    ...
    base_confidence = abs(score - 0.5) * 2.0
    if ai_results and "confidence" in ai_results:
        confidence = (base_confidence + ai_results["confidence"]) / 2.0
    else:
